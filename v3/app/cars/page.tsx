...
export default async function Cars(){
 ...
 return <main className="showroomPage">
  ...
  <section className="shell showroomContent">
   <div className="showroomCount"><span>{cars.length} vehicle{cars.length===1?'':'s'} advertised</span><span>Live website stock + current adverts</span></div>
   <div className="showroomGrid">{cars.map((c:any)=><article className={`showroomCard ${c.externalOnly?'advertCard':''}`} key={c.slug}>
    <Link href={`/cars/${c.slug}`} className="showroomImage">{c.image_urls?.[0]?<img src={c.image_urls[0]} alt={`${c.make} ${c.model}`}/>:<div className="advertNoPhoto"><span>L.A MOTORS</span><strong>{c.make}</strong><small>Tap for full advert</small></div>}<span className={`showroomBadge ${c.status}`}>{c.status==='reserved'?'Reserved':'Available'}</span></Link>
